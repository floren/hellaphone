...
static int rotation_opt = 0;
char*	mousefile = "/dev/input/event3";
char type = 'c';
...
static int xmin = 0;
static int xadjust = 0; //shifts mouse left
static int lowx = 0; //should be xmin + xadjust
static int ymin = 0;
static int yadjust = 0; //shifts mouse up //negative value helps with keyboard at the bottom of the screen
static int lowy = 0; //should be ymin + yadjust
...
setpointer ( int x, int y )
{
	...
	pointerposition.y = y;
	unlock ( &pointerlock );
	for ( i = 0; i < pointerheight && fb < framebuffer + depth*Xsize*Ysize; i++ ) {
	    memcpy ( fb, screen, width );
	    fb += bpl;
	    screen += bpl;
	}
